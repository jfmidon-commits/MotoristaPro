...
export function reconcileRide(
  offer: Pick<RideOffer, "offered_amount" | "total_expected_distance_km" | "total_expected_duration_minutes">,
  ...
  const durationDeltaMinutes = offer.total_expected_duration_minutes != null && result.actual_duration_minutes != null
    ...
    : null;
  const amountDeltaCents = result.final_amount - offer.offered_amount;
  ...
  const amountDeltaPercent = offer.offered_amount > 0 ? (amountDeltaCents / offer.offered_amount) * 100 : null;
  ...
  return {
    ...
    amountDeltaCents,
    amountDeltaPercent,
    offeredGrossPerKmCents: rate(offer.offered_amount, offer.total_expected_distance_km),
