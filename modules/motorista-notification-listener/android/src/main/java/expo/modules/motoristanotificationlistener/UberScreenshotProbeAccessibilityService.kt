...
/**
 * Experimental, privacy-safe probe used only to answer one question:
 * can AccessibilityService.takeScreenshot() obtain the Uber offer screen?
 *
 * It never persists image pixels. Only SUCCESS/ERROR plus dimensions are stored
 * as a synthetic diagnostic snapshot, so the existing diagnostics screen can
 * display the result without adding OCR yet.
 */
class UberScreenshotProbeAccessibilityService : AccessibilityService() {
  companion object {
